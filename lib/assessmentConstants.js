...
// Monthly Tests — each subject can have several test rows (Max/Obtained per
// test, not one rating for the whole subject), added/removed independently
// per subject via "+ Add Row". A rotating color per subject header bar
// (cycled by index, not tied to the subject name) just for quick visual
// separation between subject blocks.
export const SUBJECT_HEADER_COLORS = ['bg-teal-800', 'bg-indigo-800', 'bg-teal-600', 'bg-cyan-600', 'bg-emerald-700', 'bg-blue-700'];

export const WORK_COMPLETION_OPTIONS = ['Complete', 'Partial', 'Incomplete'];

// "Subject Enrichment" — an out-of-10 rating, e.g. participation/engagement
// beyond the test score itself.
export const SUBJECT_ENRICHMENT_OPTIONS = Array.from({ length: 10 }, (_, i) => `${i + 1}/10`);

// Percentage color tiers for each test row's live % badge — same thresholds
// as attendanceRatingFor (AssessmentWizard.jsx) for a consistent "green
// means good" convention across the wizard.
export function testPercentColor(percent) {
  if (percent == null) return 'bg-gray-100 text-gray-500';
  if (percent >= 90) return 'bg-green-100 text-green-700';
  if (percent >= 75) return 'bg-blue-100 text-blue-700';
