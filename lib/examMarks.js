...
export async function saveExamMarks(scheduleId, rows, currentUser, { submit = false } = {}) {
  ...
  // Only a Teacher's own Submit is admin-actionable news — an admin's own
  // save (draft or otherwise) is their own action, nothing to notify them
  // about.
  if (submit && currentUser.role === 'Teacher') {
    const sectionLabel = schedule.sectionName ? ` - ${schedule.sectionName}` : '';
    createAdminNotification(schoolId, {
      type: 'exam',
      title: 'Marks submitted for verification',
      message: `${currentUser.name} submitted ${schedule.subject} marks for ${schedule.className}${sectionLabel} — ${results.length} student(s).`,
      link: `/dashboard/exams/${schedule.examId}/verify`,
      sourceId: scheduleId,
    }).catch((err) => console.error('createAdminNotification failed', err));
  }
...
// Was: a schedule × 3-query loop — for a school with 15 subjects across 10
// sections on one ended exam, ~450 sequential round-trips on a single
// dashboard load. Every per-schedule query is now one batched query across
// every schedule instead: student counts are computed in memory from a
// single candidate-student fetch (schoolId/status/academicSession/class
// already narrow it to a small, relevant set — not the whole school), and
// the two `examMark`/`examOptionalEnrollment` counts become one `groupBy`
// each, keyed by `examScheduleId`.
export async function getPendingMarksAlerts() {
  ...
  const schedules = endedExams.flatMap((exam) =>
    exam.schedules.map((schedule) => ({ ...schedule, academicSession: exam.academicSession }))
  );
  if (schedules.length === 0) return [];

  const scheduleIds = schedules.map((s) => s.id);
  const academicSessions = [...new Set(schedules.map((s) => s.academicSession))];
  const classNames = [...new Set(schedules.map((s) => s.className))];

  const [candidateStudents, enteredGroups, enrolledGroups] = await Promise.all([
    prisma.student.findMany({
      where: { schoolId, status: 'Active', academicSession: { in: academicSessions }, class: { in: classNames } },
      select: { academicSession: true, class: true, section: true },
    }),
    ...
    prisma.examOptionalEnrollment.groupBy({
      by: ['examScheduleId'],
  ...
  const enteredCountByScheduleId = new Map(enteredGroups.map((g) => [g.examScheduleId, g._count.id]));
  const enrolledCountByScheduleId = new Map(enrolledGroups.map((g) => [g.examScheduleId, g._count.id]));

  const pendingByClass = new Map();
  for (const schedule of schedules) {
    const classStudentCount = candidateStudents.filter(
      (s) =>
        s.academicSession === schedule.academicSession &&
        s.class === schedule.className &&
        (!schedule.sectionName || s.section === schedule.sectionName)
    ).length;
    const enteredCount = enteredCountByScheduleId.get(schedule.id) || 0;
    const enrolledCount = enrolledCountByScheduleId.get(schedule.id) || 0;
...
// per ExamSchedule with total/entered/pending/absent counts. Was the same
// per-schedule 3-query loop as getPendingMarksAlerts above — same fix: one
// candidate-student fetch + one examMark fetch + one groupBy, all batched
// across every schedule instead of one round-trip per schedule.
export async function getMarksEntryProgress(examId) {
  ...
  const [candidateStudents, allMarks, enrolledGroups] = await Promise.all([
    prisma.student.findMany({
      where: { schoolId, status: 'Active', academicSession: exam?.academicSession, class: { in: classNames } },
      select: { class: true, section: true },
    }),
    prisma.examMark.findMany({
      where: { examScheduleId: { in: scheduleIds } },
      select: { examScheduleId: true, marksObtained: true, gradeValue: true, remarksValue: true, practicalMarksObtained: true, isAbsent: true },
    }),
    prisma.examOptionalEnrollment.groupBy({
      by: ['examScheduleId'],
      where: { examScheduleId: { in: scheduleIds } },
  ...
  const marksByScheduleId = new Map();
  for (const mark of allMarks) {
    const list = marksByScheduleId.get(mark.examScheduleId) || [];
    list.push(mark);
    marksByScheduleId.set(mark.examScheduleId, list);
  }
  const enrolledCountByScheduleId = new Map(enrolledGroups.map((g) => [g.examScheduleId, g._count.id]));
  ...
  for (const schedule of schedules) {
    const classStudentCount = candidateStudents.filter(
      (s) => s.class === schedule.className && (!schedule.sectionName || s.section === schedule.sectionName)
    ).length;
    ...
    const totalStudents = schedule.isOptional ? enrolledCountByScheduleId.get(schedule.id) || 0 : classStudentCount;

    const marks = marksByScheduleId.get(schedule.id) || [];
