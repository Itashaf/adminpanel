...
export async function getLeaveBalance(teacherId = '') {
  const params = teacherId ? `?teacherId=${teacherId}` : '';
  const res = await fetch(`/api/leaves/balance${params}`);
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to load leave balance');
  }
  return res.json();
}

export async function getStaffAttendance(date) {
  const res = await fetch(`/api/staff-attendance?date=${date}`);
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to load staff attendance');
  }
  return res.json();
}

export async function saveStaffAttendance(date, records) {
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to save staff attendance');
  }
  return res.json();
}

export async function getMyCheckInStatus() {
  const res = await fetch('/api/attendance/check-in');
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to load check-in status');
  }
  return res.json();
}
...
// Same GET /api/teacher/notifications... routes the mobile app already
// uses (requireTeacher() accepts both the web cookie and mobile's Bearer
// header) — reading/marking read here writes the exact same
// TeacherNotification row the mobile app reads, so "read" on one is "read"
// on the other with no extra sync code needed.
export async function getTeacherNotifications(page = 1) {
  const res = await fetch(`/api/teacher/notifications?page=${page}`);
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to load notifications');
  }
...
export async function markAllTeacherNotificationsRead() {
  const res = await fetch('/api/teacher/notifications/read-all', { method: 'POST' });
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to update notifications');
  }
...
export async function getClassAssessments({
  ...
  const params = new URLSearchParams({ class: className, section: sectionName, session: academicSession, month, year, page, pageSize, status, sort });
  if (search) params.set('search', search);
...
export async function getStudentAssessment(studentId, month, year) {
  const res = await fetch(`/api/assessments/student/${studentId}?month=${month}&year=${year}`);
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
    throw new Error(error.error || 'Failed to load assessment');
  }
  return res.json();
}

export async function saveStudentAssessment(studentId, month, year, data, submit = false) {
  const res = await fetch(`/api/assessments/student/${studentId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ month, year, submit, ...data }),
  });
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
...
export async function getClassAssessmentSummary({ className, sectionName, academicSession, month, year, page = 1, pageSize = 20 }) {
  const params = new URLSearchParams({ class: className, section: sectionName, session: academicSession, month, year, page, pageSize });
  const res = await fetch(`/api/assessments/summary?${params.toString()}`);
  if (!res.ok) {
    const error = await res.json().catch(() => ({}));
