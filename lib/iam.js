...
export async function getCurrentUserInfo() {
  ...
  if (session?.role === 'Parent') {
    ...
    if (account && account.students.length > 0) {
      ...
      // For the mobile app's dashboard header (school name + this school's
      // own logo, not the app's own generic logo) — nothing else on this
      // branch needed it before.
  ...
  if (session?.role === 'Teacher') {
    ...
    if (teacher) {
      ...
      // For the mobile app's dashboard header (school name + this school's
      // own logo next to it, not the app's own generic logo) — nothing
      // else on this branch needed it before.
      const school = await prisma.school.findUnique({
        where: { id: session.schoolId },
        select: { displayName: true, logoUrl: true },
      });
