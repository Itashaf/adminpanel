...
// `days` is every date in the month this teacher actually has a marked
// record for — {date, status} — so a calendar view can dot only marked days
// instead of guessing "Not Marked" vs "Absent" for a date the school never
// opened attendance for.
export async function getTeacherAttendanceSummary(teacherId, schoolId, monthPrefix) {
  ...
  rows.forEach((row) => {
    ...
    days.push({ date: row.date, status: record.status });
  });
