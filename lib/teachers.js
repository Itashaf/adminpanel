...
// so a brand new school still starts with zero rows and ABC Public School's
// roster (seeded via prisma/seed.js from lib/teacherSeedData.js) is only
// ever visible while it's the active school.
// `schoolId` is optional — pass it explicitly when you already have a
// resolved session (see lib/iam.js's getCurrentUserInfo(), which passes a
// signed-in mobile Teacher's own session.schoolId here). This file can't
// resolve the session itself: it also exports pure constants (ACADEMIC_SESSIONS...)
// that Client Components import directly, and next/headers-dependent code
// can never live anywhere in a module those components pull in.
async function getSchoolTeachers(schoolId = getActiveSchoolId()) {
  const rows = await prisma.teacher.findMany({ where: { schoolId } });
  return rows.map(decorateTeacher);
}

export async function getTeacherStats(schoolId = getActiveSchoolId()) {
...
export async function getTeacherById(id, schoolId = getActiveSchoolId()) {
...
export async function addTeacher(data, schoolId = getActiveSchoolId()) {
...
export async function updateTeacher(id, data, schoolId = getActiveSchoolId()) {
...
export async function updateTeacherStatus(id, status, schoolId = getActiveSchoolId()) {
...
export async function resetTeacherPassword(id, schoolId = getActiveSchoolId()) {
  const existing = await getTeacherById(id, schoolId);
...
export async function assignClassToTeacher(id, { academicSession, class: className, section, subject }, schoolId = getActiveSchoolId()) {
  const existing = await getTeacherById(id, schoolId);
  if (!existing) return null;
...
export async function removeAssignmentFromTeacher(id, assignmentId, schoolId = getActiveSchoolId()) {
  const existing = await getTeacherById(id, schoolId);
  if (!existing) return null;
