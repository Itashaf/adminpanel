...
import { isClassInTeacherScope, getTeacherClassScope } from './roleGuard';
...
export async function getVisibleNotices(currentUser) {
  ...
  if (currentUser.role === 'Teacher') {
    // getTeacherClassScope merges in classTeacherOf too — assignedClasses
    // alone left a Class Teacher with zero subject assignments of their own
    // unable to see notices posted for their own homeroom class.
    const scope = getTeacherClassScope(currentUser);
    return all.filter((notice) => {
      if (notice.audience === 'Whole School') return true;
      return isClassInTeacherScope(scope, notice.academicSession, notice.className, notice.sectionName);
    });
  }
...
export async function getNoticeById(id, currentUser) {
  ...
  if (currentUser.role === 'Teacher') {
    const inScope = isClassInTeacherScope(
      getTeacherClassScope(currentUser),
      notice.academicSession,
      notice.className,
      notice.sectionName
    );
...
// An Admin can manage any notice. A Teacher may only manage (edit/delete) a
// notice they themselves posted — as their own class's Class Teacher, never
// an Admin's notice or another teacher's (unlike Homework, ownership here is
// checked against postedByTeacherId the same way canManageHomework checks
// assignedByTeacherId).
export function canManageNotice(currentUser, notice) {
  if (currentUser.role !== 'Teacher') return true;
  return notice.postedByTeacherId === currentUser.teacherId;
}

// A Teacher may only ever post a "Class" notice to exactly one of their own
// Class-Teacher sections (Section.classTeacherId) — never "Whole School",
// never a class they merely teach a subject in, never another section of
// the same class. This deliberately checks currentUser.classTeacherOf, not
// getTeacherClassScope's merged assignments+classTeacherOf — Notices is a
// stricter, class-teacher-only privilege, unlike Homework which any subject
// teacher can also post for their own class+section.
function assertScopeAllowed(data, currentUser) {
  ...
  if (data.audience !== 'Class' || !data.sectionName) {
    throw new Error('As a Class Teacher, you can only send a notice to your own class and section.');
  }
  const isOwnSection = isClassInTeacherScope(
  ...
  if (!isOwnSection) {
    throw new Error('You can only send a notice to your own class and section.');
  }
}
...
export async function createNotice(data, currentUser) {
  ...
  // Only a Teacher posting is admin-actionable news — an admin's own notice
  // is their own action, nothing to notify them about.
  if (currentUser.role === 'Teacher') {
    createAdminNotification(schoolId, {
      type: 'notice',
      title: 'New notice from a Class Teacher',
      message: `${currentUser.name} posted "${notice.title}" for ${notice.className}${notice.sectionName ? ` - ${notice.sectionName}` : ''}.`,
...
export async function updateNotice(id, data, currentUser) {
  ...
  if (!canManageNotice(currentUser, existing)) {
    throw new Error('You can only edit notices you posted.');
  }
  assertScopeAllowed(data, currentUser);
...
export async function deleteNotice(id, currentUser) {
  ...
  if (!canManageNotice(currentUser, existing)) {
    throw new Error('You can only delete notices you posted.');
  }
