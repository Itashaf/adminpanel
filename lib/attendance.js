...
// Was: fetch the WHOLE school's students (every Json blob included) then
// filter down to one class+section in JS — for a 2,000-student school, a
// single class's attendance roster paid the cost of downloading everyone.
// Scoped directly at the query instead, with only the columns this roster
// actually renders.
export async function getStudentsForClassSection(className, sectionName, academicSession) {
  ...
  const students = await prisma.student.findMany({
    ...
    select: { id: true, firstName: true, lastName: true, initials: true, admissionId: true },
  });
...
// The fields every attendance report function below actually reads off a
// student row — never the full Student shape (guardian/fee/address/aadhaar
// Json blobs included), which `getAllStudents` used to hand back for what
// was really just a filter-then-discard pass over the whole school.
...
// Was: fetch the WHOLE school's students, then filter every one of these
// down to size in JS. Every filter the caller can supply is now pushed into
// the `where` clause instead, so a single class's report no longer pays for
// downloading the rest of the school first.
async function relevantStudents({ academicSession, classFilter, sectionFilter, genderFilter, search, allowedClasses }) {
  const schoolId = await resolveSchoolId();
  const conditions = [{ schoolId, academicSession, status: 'Active' }];

  if (allowedClasses) conditions.push({ class: { in: allowedClasses } });
  if (classFilter && classFilter !== 'All Classes') conditions.push({ class: classFilter });
  if (sectionFilter && sectionFilter !== 'All Sections') conditions.push({ section: sectionFilter });
  if (genderFilter && genderFilter !== 'All') conditions.push({ gender: genderFilter });
  ...
  return prisma.student.findMany({ where: { AND: conditions }, select: REPORT_STUDENT_SELECT });
}
...
// `knownStudent` — pass { class, section, academicSession } when the caller
// already has the student record (e.g. getAssessmentForStudent, which loads
// it right before calling this) to skip a redundant findFirst round trip.
// Every other caller omits it and this looks the student up itself as before.
export async function getStudentAttendanceStats(studentId, { academicSession, month, knownStudent } = {}) {
  const schoolId = await resolveSchoolId();
  const student = knownStudent || (await prisma.student.findFirst({ where: { id: studentId, schoolId }, select: REPORT_STUDENT_SELECT }));
...
export async function getStudentAttendanceSessionStats(studentId) {
  const schoolId = await resolveSchoolId();
  const student = await prisma.student.findFirst({ where: { id: studentId, schoolId }, select: REPORT_STUDENT_SELECT });
  if (!student) return null;
