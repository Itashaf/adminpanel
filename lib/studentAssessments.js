...
// A test row has Max/Obtained, not a single rating (see AssessmentWizard.jsx's
// AcademicsStep) — this buckets each test's % into the same RATING_LEVELS
// scale for Reports' academicCounts, same thresholds as the wizard's own
// attendanceRatingFor/testPercentColor for consistency.
function academicRatingFor(percent) {
...
export async function getAssessmentsForClass(
  ...
  const roster = pageStudents.map((s) => {
    ...
    return {
      ...
      sections: {
        academic: (assessment?.academics || []).some((row) => row.tests?.length > 0),
...
export async function getAssessmentForStudent(studentId, month, year, currentUser) {
  ...
  const [existing, attendanceStats, examResults, subjectNames] = await Promise.all([
...
export async function getClassAssessmentSummary(currentUser, { className, sectionName, academicSession, month, year, page = 1, pageSize = 20 }) {
  ...
  const perStudent = students.map((s) => {
    ...
    if (a) {
      ...
      // Tallied per test row (a subject can have several tests in a month
      // now), not per subject — each test's % bucketed via academicRatingFor.
      (a.academics || []).forEach((row) => {
        (row.tests || []).forEach((t) => {
          ...
          const rating = academicRatingFor(percent);
          if (rating && academicCounts[rating] !== undefined) academicCounts[rating] += 1;
        });
      });
