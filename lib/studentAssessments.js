...
export async function getAssessmentsForClass(
  currentUser,
  { className, sectionName, academicSession, month, year, page = 1, pageSize = 20, search = '' }
) {
  ...
  const [activeStudentIds, total, pageStudents, assessments] = await Promise.all([
...
export async function getClassAssessmentSummary(currentUser, { className, sectionName, academicSession, month, year }) {
  assertClassInScope(currentUser, className, sectionName, academicSession);
  const schoolId = await resolveSchoolId();
