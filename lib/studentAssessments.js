...
import { getSubjectsForClass } from './classes';
import { getPublishedResultsForStudent } from './examResults';
import { isClassInTeacherScope } from './roleGuard';
import { FALLBACK_SUBJECTS, RATING_LEVELS, ACTIVITY_OPTIONS, BEHAVIOUR_CATEGORIES } from './assessmentConstants';
...
function decorate(row) {
  return {
    ...
    attendanceDetail: row.attendanceDetail || { totalWorkingDays: '', daysPresent: '', remark: '' },
    overallPerformance: row.overallPerformance,
    overallTags: row.overallTags || [],
    overallRemark: row.overallRemark,
    conciseReport: row.conciseReport || {
      parentInformed: '',
      dateInformed: '',
      ptmAttended: '',
      healthStatus: '',
    ...
    // Array of { type, option, achievement } entries — a student can have
    // several (the step's "Add Another Activity" button), not just one.
...
// A Teacher only ever sees/assesses students in a class they're the Class
// Teacher of (Section.classTeacherId) — deliberately currentUser.classTeacherOf
// directly, NOT getTeacherClassScope's merged assignments+classTeacherOf.
// Per the module's own "Class Teacher: full access / Teacher: only own
// class" note, a Teacher merely assigned a subject in a class (no
// classTeacherOf entry for it) must not see that class here at all — same
// class-teacher-only privilege Notices uses for posting (see
// lib/notices.js's assertScopeAllowed), just applied to viewing too, not
// only writing. Throws (never silently returns nothing) so a route can
// turn it into a 403.
function assertClassInScope(currentUser, className, sectionName, academicSession) {
  ...
  // Every route calling into this only checks getCurrentUserInfo() for
  // "signed in at all", never role — a real Parent session (or any other
  // non-admin/non-Teacher role) used to fall through the old `role !==
  // 'Teacher'` early-return below with full, unrestricted read/write on
  // every student's assessment in the school. Reject explicitly instead of
  // only handling the Teacher case.
  if (currentUser.role !== 'Teacher') {
    throw new Error('Forbidden');
  }
  const scope = currentUser.classTeacherOf || [];
...
export async function getAssessmentsForClass(
  ...
  // Assessment rows and the active-student id list don't depend on the
  // search/status filter below, so both run alongside it instead of before
  // it — one round trip's worth of latency saved per request.
  const [assessments, activeStudentIds] = await Promise.all([
    // Both the class-wide stats and the "Completed"/"Pending" tab filters
    // (below) key off the same completed-id set, so a Not Started student
    // (no row at all) and a Draft row both land in "Pending", matching the
    // tab counts.
    prisma.studentAssessment.findMany({ where: { schoolId, className, sectionName, month, year } }),
  ...
  const [total, pageStudents] = await Promise.all([
  ...
  const roster = pageStudents.map((s) => {
    ...
    return {
      ...
      sections: {
        ...
        participation: (assessment?.activities || []).length > 0,
      },
    };
  });
  ...
  // 'Needs Attention' was never a real value — RATING_LEVELS only ever
  // writes 'Needs Improvement' (see assessmentConstants.js) — so this stat
  // was permanently 0 regardless of how many students were actually rated
  // that way.
  const needsAttention = classAssessments.filter((a) => a.overallPerformance === 'Needs Improvement').length;
...
export async function getAssessmentForStudent(studentId, month, year, currentUser) {
  ...
  // All 4 run in parallel, not sequentially — attendanceStats reuses the
  // student record already fetched above (knownStudent) instead of paying
  // for a second, redundant findFirst inside getStudentAttendanceStats.
  const [existing, attendanceStats, examResults, classSubjects, schoolSubjects] = await Promise.all([
    ...
    getStudentAttendanceStats(studentId, {
      ...
      knownStudent: student,
    }).catch(() => null),
    getPublishedResultsForStudent(studentId).catch(() => []),
    // Only the subjects actually assigned to this student's class (Class.subjects)
    // — not the school's whole master subject list — so a Class 1 student's
    // Academic Performance step doesn't show Physics/Chemistry etc.
    getSubjectsForClass(student.class, student.academicSession),
    getSubjectNames(),
  ]);
  ...
  return {
    ...
    autoFill: {
      ...
      // NA days don't count either way (see lib/attendance.js's summarize) —
      // `total - NA` is the actual working-day count for the Attendance
      // step's Total Working Days / Days Present fields, which start out
      // auto-filled from real attendance but stay editable (a teacher can
      // correct them for a day the records don't reflect yet).
      totalWorkingDays: attendanceStats ? attendanceStats.total - attendanceStats.NA : null,
      daysPresent: attendanceStats?.Present ?? null,
...
export async function saveAssessment(studentId, month, year, data, currentUser, { submit = false } = {}) {
  const schoolId = await resolveSchoolId();
  // Both run in parallel — `existing` doesn't depend on the student record,
  // just studentId/month/year. This is the autosave path (called every
  // ~1.5s while a teacher is filling out a step), so the round trip saved
  // here matters more than most.
  ...
  // A COMPLETED assessment used to keep accepting writes here — `fields`
  // was always applied regardless of `existing.status`, only `statusFields`
  // cared whether it was already submitted. The Wizard's autosave (every
  // ~1.5s) keeps firing after Submit, so reopening an already-"locked"
  // report silently drifted its content away from what was actually signed
  // off, with `submittedByName`/`submittedAt` staying stale and no
  // audit trail. One-way lock instead, same convention as TeacherLeave's
  // Pending→Approved/Rejected transition (no revising after the fact).
  if (existing?.status === 'COMPLETED') {
    throw new Error('This assessment has already been submitted and can no longer be edited.');
  }
  ...
  const fields = {
    ...
    attendanceDetail: data.attendanceDetail ?? existing?.attendanceDetail ?? {},
    ...
    conciseReport: data.conciseReport ?? existing?.conciseReport ?? {},
    ...
    // Shallow-merged, not replaced — parentCommunication holds several
    // independent named fields (parentContacted, keyConcern,
    // specificIntervention, targetOutcome, parentInvolvementNotes).
    // QuickAssessmentModal/bulk actions only ever send `{parentContacted}`
    // (that's the only field they show), and a full replace here silently
    // blanked out keyConcern/specificIntervention/targetOutcome/
    // parentInvolvementNotes a teacher had already written in the full
    // Wizard — fields the quick tools never even display. The full Wizard
    // itself is unaffected: it always sends the complete object anyway.
    parentCommunication: { ...(existing?.parentCommunication || {}), ...(data.parentCommunication || {}) },
  };
...
export async function getClassAssessmentSummary(currentUser, { className, sectionName, academicSession, month, year, page = 1, pageSize = 20 }) {
  ...
  // Neither query depends on the other's result — assessments filters by
  // className/sectionName directly (denormalized on StudentAssessment, same
  // as getAssessmentsForClass above), not by a student-id list, so both run
  // in parallel instead of assessments waiting on students first.
  ...
  // overallPerformance ("Overall Progress" on the Concise Report step) uses
  // RATING_LEVELS now (Excellent/Good/Average/Needs Improvement) — same
  // 4-level scale the Bulk Assessment Mode picker and "Complete in 30s"
  // quick modal also write to this field, so all three stay consistent.
  const overallCounts = Object.fromEntries(RATING_LEVELS.map((o) => [o, 0]));
  ...
  const perStudent = students.map((s) => {
    ...
    if (a) {
      ...
      // Only fixed (Competition/Olympiad/Sports) options are tallied — an
      // "Activity" type entry's option is free text, no fixed count bucket.
      (Array.isArray(a.activities) ? a.activities : []).forEach((entry) => {
        if (activityCounts[entry.option] !== undefined) activityCounts[entry.option] += 1;
      });
    }
    return {
      name: `${s.firstName} ${s.lastName}`,
      ...
      parentContacted: a?.conciseReport?.parentInformed || '',
    };
  });
  ...
  return {
    totals: {
      ...
      good: overallCounts['Good'] || 0,
      average: overallCounts['Average'] || 0,
      needsAttention: overallCounts['Needs Improvement'] || 0,
    },
    overallCounts,
    behaviourCounts,
    academicCounts,
...
export async function getRecentlyAssessed(currentUser, limit = 5) {
  // Same authorization boundary as assertClassInScope — a signed-in Parent
  // (or any other non-admin/non-Teacher role) used to hit the `: rows`
  // branch below with zero filtering, seeing every student's assessment
  // school-wide.
  if (currentUser.role !== 'SchoolAdmin' && currentUser.role !== 'SuperAdmin' && currentUser.role !== 'Teacher') {
    throw new Error('Forbidden');
  }
  const schoolId = await resolveSchoolId();
  const isTeacher = currentUser.role === 'Teacher';
  // Same class-teacher-only scope as assertClassInScope above — not
  // getTeacherClassScope's merged assignments+classTeacherOf.
  const scope = isTeacher ? currentUser.classTeacherOf || [] : [];
