...
// The subject list actually assigned to one class (Class.subjects), for
// modules that need "only what this class is taught" rather than the
// school's whole master subject list (lib/subjects.js's getSubjectNames) —
// e.g. the Monthly Assessment wizard's Academic Performance step. Falls
// back to an empty array (caller decides its own fallback) if the class
// isn't found or has no subjects configured yet.
export async function getSubjectsForClass(className, academicSession) {
  const schoolId = await resolveSchoolId();
  const cls = await prisma.class.findFirst({
    where: { schoolId, academicSession, name: className },
