...
// roster (seeded via prisma/seed.js from lib/studentSeedData.js) is only
// ever visible while it's the active school.
// `schoolId` is optional — pass it explicitly when you already have a
// resolved session (a mobile Teacher's own school, say; see
// lib/auth/schoolContext.js's resolveSchoolId(), used by lib/attendance.js
// and lib/iam.js) — this file can't resolve that itself since it also
// exports pure constants that Client Components import directly, and
// next/headers-dependent code can never live anywhere in a module those
// components pull in.
async function getSchoolStudents(schoolId = getActiveSchoolId()) {
  const rows = await prisma.student.findMany({ where: { schoolId } });
  return rows.map(decorateStudent);
}

export async function getStudentStats(schoolId = getActiveSchoolId()) {
...
export async function getStudentById(id, schoolId = getActiveSchoolId()) {
...
export async function addStudent(data, schoolId = getActiveSchoolId()) {
...
export async function bulkCreateStudents(rows, schoolId = getActiveSchoolId()) {
...
export async function updateStudent(id, data, schoolId = getActiveSchoolId()) {
...
export async function deleteStudent(id, schoolId = getActiveSchoolId()) {
  const existing = await getStudentById(id, schoolId);
  if (!existing) return null;
