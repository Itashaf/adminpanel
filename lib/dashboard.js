...
// A Teacher's dashboard shows only what's theirs — their own Class Teacher
...
// on the admin dashboard above. "My Classes" (classRows below) is
// deliberately currentUser.classTeacherOf directly, NOT
// getTeacherClassScope's merged assignments+classTeacherOf — it's an
// attendance widget (per-class "Mark Attendance"/"Pending Today"), and
// Attendance only ever belongs to the Class Teacher, same as
// app/api/attendance/route.js. A class the Teacher merely teaches a subject
// in must not show up here.
export async function getTeacherDashboardOverview(currentUser) {
  const assignedClasses = currentUser.classTeacherOf || [];
